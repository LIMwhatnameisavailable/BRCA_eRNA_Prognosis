...
# Rename downloaded file to 'L1000_Result.csv' and place in 'Results/Drug_Analysis/' folder
risk_data <- read.csv("Results/risk_data.csv")
all_genes_diff <- read.csv("Results/all_genes_diff.csv", row.names = 1)
pred_ic50 <- read.csv("Results/Drug_Analysis/DrugPredictions.csv", row.names = 1)
expr_final <- read.csv("Results/expr_final.csv", row.names = 1)
...
# Match actual names in column names
...
# Extract L1000FWD Top 8 reversal drugs
lincs_file <- "Results/Drug_Analysis/L1000_Result.csv"
...
if(file.exists(lincs_file)){
  ...
  top_l1000 <- raw_lincs %>%
    ...
    filter(Score < 0) %>% # Keep only negative scores
    group_by(Drug) %>% 
...
# Extract GDSC Top 8 high-risk sensitive drugs
...
# Loop to calculate differences and P-values for all drugs
res_list <- list()
...
# Filter Top 8 most significant drugs
top_gdsc <- gdsc_df %>%
  arrange(Pval, Diff) %>% 
  head(8) %>%
  mutate(
    ...
    Mechanism_of_Action_Target = "" # Leave blank for manual filling
  ) %>%
  dplyr::select(Candidate_Drug, Screening_Approach, Key_Metric, Mechanism_of_Action_Target)

# Merge and export table
