...
# Part 1: Gene expression matrix preprocessing
...
# Check number of overlapping IDs
...
# Part 2: Risk model calculation and sample grouping
...
# Part 3: Gene ranking using limma

# Match samples between expression matrix and risk groups
...
# Ensure identical sample ordering
...
# Linear model fitting and empirical Bayes smoothing
fit <- lmFit(expr_filtered, design)
fit2 <- contrasts.fit(fit, contrast.matrix)
fit2 <- eBayes(fit2)

# Extract results and construct GSEA ranked list
...
gene_ranks <- sort(gene_ranks, decreasing = TRUE) # t > 0 indicates higher expression in High risk group


# Part 4: Perform GSEA analysis
